...
//        .toLocalTime();
        ...
        for(UserMealWithExceed mealWithExceed: filteredList){
            System.out.println(mealWithExceed);
        }
    }

    public static List<UserMealWithExceed>  getFilteredWithExceeded(List<UserMeal> mealList, LocalTime startTime, LocalTime endTime, int caloriesPerDay) {
        Map<LocalDate, Integer> caloriesPerDayEaten = mealList
                .stream()
                .collect(
        ...
        List<UserMealWithExceed> res2 = mealList.stream()
                .filter(m -> TimeUtil.isBetween(m.getTime(), startTime, endTime))
                .map(m -> new UserMealWithExceed(m, caloriesPerDayEaten.get(m.getDate()) > caloriesPerDay))
